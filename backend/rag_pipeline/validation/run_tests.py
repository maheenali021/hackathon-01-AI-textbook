#!/usr/bin/env python3
"""
Test runner for the retrieval validation system
...
def run_tests():
    ...
    print("Running tests for retrieval validation system...")
    ...
    cmd = [
        ...
        f"--cov-fail-under=85",  # Fail if coverage is below 85%
...
def run_unit_tests():
    ...
    cmd = [
        sys.executable, "-m", "pytest",
        "test_retrieval_validation.py",  # Only run unit tests
        "-v",
        "--cov=..",
...
def main():
    ...
    print("=" * 60)
    print("RETRIEVAL VALIDATION SYSTEM - TEST RUNNER")
    print("=" * 60)
    ...
    if current_dir != validation_dir:
        os.chdir(validation_dir)

    print(f"Current directory: {os.getcwd()}")

    # Check if required modules exist
    required_dirs = ["services", "models", "api", "utils", "tests"]
    missing_dirs = []

    for dir_name in required_dirs:
        if not (validation_dir / dir_name).exists():
